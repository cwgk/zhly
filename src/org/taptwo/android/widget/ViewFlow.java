...
public class ViewFlow extends AdapterView<Adapter> {
	...
	private final static int VIEW_FLOW_ANIMA = 0;
	private final static int FLOW_INDICATOR_ANIMA = 1;
	...
	public ViewFlow(Context context, AttributeSet attrs) {
		...
		mSideBuffer = 0;//styledAttrs.getInt(R.styleable.ViewFlow_sidebuffer, 0);//设置初始隐藏view数目
		init();
	}
...
//						break;
	...
	public void stopAutoFlowTimer(){
		if(handler!=null)
			handler.removeMessages(VIEW_FLOW_ANIMA);
		handler = null;
	}
...
//				}
	...
	protected void onScrollChanged(int h, int v, int oldh, int oldv) {
		...
		if (mIndicator != null) {
			...
			int hPerceived = h + (mCurrentAdapterIndex - mCurrentBufferIndex)
					* getWidth();
			//Log.i("Tagg", "onScrollChanged h:"+ hPerceived);
...
//		Message message = handler.obtainMessage(FLOW_INDICATOR_ANIMA);
//		Bundle bundle=new Bundle();
//		int hPerceived = getScrollX() + (mCurrentAdapterIndex - mCurrentBufferIndex)
//				* getWidth();
//		//bundle.putInt("ScrollX", getScrollX());
//		bundle.putInt("ScrollX",newX);
//		message.setData(bundle);
//		handler.sendMessageDelayed(message, Math.abs(delta) * 2);
	}
...
//			}
	...
	public void setViewCounts(int mViewCounts) {
